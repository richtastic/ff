...
R3D.BA.scoreForPointMatch = function(viewA,pointA, viewB,pointB, scale,angle, cellScale){
	cellScale = cellScale!==undefined ? cellScale : 1.0;
	var compareSize = 11;
	var imageA = viewA.image();
	var imageB = viewA.image();
	
	var cellSize = Math.round((viewA.pixelsCompareP2D()+viewB.pixelsCompareP2D())*0.5);
	var cellScale = cellScale * (compareSize/cellSize);

	var matrix = new Matrix(3,3).identity();
		matrix = Matrix.transform2DScale(matrix,scale);
		matrix = Matrix.transform2DRotate(matrix,angle);
	var needleA = imageA.extractRectFromFloatImage(pointA.x,pointA.y,cellScale,null,compareSize,compareSize, matrix);
		matrix = new Matrix(3,3).identity();
	var needleB = imageA.extractRectFromFloatImage(pointB.x,pointB.y,cellScale,null,compareSize,compareSize, matrix);
	var scores = R3D.searchNeedleHaystackImageFlatTest2(needleA, null, needleB, true);
...
R3D.BA.World.prototype.addMatchForViews = function(viewA,pointA, viewB,pointB, relativeScale,relativeAngle){
	...
	var imageB = viewB.image();
		// var seedAngles = Code.lineSpace(-20,20, 10);
		// var seedScales = Code.lineSpace(-.1,.1, .1);
		var seedAngles = Code.lineSpace(-25,25, 5);
		var seedScales = Code.lineSpace(-.1,.1, .1);
		// var seedAngles = null;
		// var seedScales = null;
	...
	var needleScore = R3D.BA.scoreForPointMatch(viewA,pointA, viewB,pointB, bestScale,bestAngle);
		needleScore = needleScore["cross"];
	var haystackScore = R3D.BA.scoreForPointMatch(viewA,pointA, viewB,pointB, bestScale,bestAngle, 0.333);
		haystackScore = haystackScore["cross"];

	var referenceA = needleScore["referenceA"];
	var referenceB = needleScore["referenceB"];

	// var relativeScore = haystackScore/needleScore;
	// console.log(relativeScore);

	// TODO: SET RANK FROM NEIGHBORHOOD:
	// var rank = relativeScore * relativeScore;

	// absolute cost * path-cost
	var rank = needleScore * haystackScore / needleScore;

	// ...
	//var rank = (needleScore*needleScore)/(referenceA*referenceB);
	// console.log(rank);

		// var scores = R3D.BA.scoreForPointMatch(viewA,pA, viewB,pB, bestScale,bestAngle);
		// var destinationScore = scores["cross"];
		// var referenceB = scores["referenceB"];
		// var referenceA = scores["referenceA"];
...
R3D.BA.View.prototype.pointsInsideCellPoint = function(point, isFinal){
	var cellSize = this.neighborhoodSize();
	...
	return this.pointsInsideRect(min, max, isFinal);
}
R3D.BA.View.prototype.pointsInsideRect = function(min,max, isFinal){
...
*/
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
R3D.BA.World.prototype.epipolarSearch = function(){
	...
	for(var i=0; i<list.length; ++i){
		...
		var pointA = match.pointForView(viewA);
		var pointB = match.pointForView(viewB);
		points2DA.push(pointA.point());
		points2DB.push(pointB.point());
	}
	// console.log(points2DA);
	var FFwd = R3D.fundamentalFromUnnormalized(points2DA,points2DB);
	...
	d.matrix().scale(sca);
	d.matrix().translate(10 + offsetX, 10);
...
var testPoints2DA = [];
for(j=0; j<15; ++j){
testPoints2DA.push(new V2D(200,25 + j*25));
// for(j=0; j<20; ++j){
...
// var checkJ = 5;
	...
	// for(j=0; j<testPoints2DA.length; ++j){
		...
		// A
		var clippedB, distanceB, closestB, lineB;
		if(a){
		...
		var clippedA, distanceA, closestA, lineA;
		if(b){
		...
		var color = new V3D(Math.random()*1.0,Math.random()*0.5,Math.random()*0.5);
		color = Code.getColARGBFromFloat(0.5,color.x,color.y,color.z);

// console.log(clippedA);
// console.log(clippedB);
// if( !clippedA || !clippedB || !(clippedB.length==0 && clippedA.length==0) ){
		...
		if(b){
			...
			d.graphics().strokeLine();
			d.graphics().endPath();
			d.matrix().translate(10 + imageWidth, 10 + 0);
			GLOBALSTAGE.addChild(d);
		}
		// lineB
		if(clippedB && clippedB.length>0){
			var d = new DO();
			d.graphics().setLine(2.0, color);
			d.graphics().beginPath();
			d.graphics().moveTo(clippedB[0].x,clippedB[0].y);
			d.graphics().lineTo(clippedB[1].x,clippedB[1].y);
			d.graphics().strokeLine();
			d.graphics().endPath();
			d.matrix().translate(10 + imageWidth, 10 + 0);
			GLOBALSTAGE.addChild(d);
		...
		// lineA
		if(clippedA && clippedA.length>0){
			var d = new DO();
			d.graphics().setLine(2.0, color);
			d.graphics().beginPath();
			d.graphics().moveTo(clippedA[0].x,clippedA[0].y);
			d.graphics().lineTo(clippedA[1].x,clippedA[1].y);
			d.graphics().strokeLine();
			d.graphics().endPath();
			d.matrix().translate(10 + 0, 10 + 0);
			GLOBALSTAGE.addChild(d);
		...
		// get a slice from A & B
	var index = -1;
	if(j==index){
		if(a&&b){
			var pointA1 = clippedA[0];
			var pointA2 = clippedA[1];
			var pointB1 = clippedB[0];
			var pointB2 = clippedB[1];
...
// TODO: ORDER / DIR DEPENDS ON WHERE EPIPOLE IS


			var lengthA = V2D.distance(pointA1,pointA2);
			var lengthB = V2D.distance(pointB1,pointB2);
			var locationA = V2D.distance(pointA1,closestA);
			var locationB = V2D.distance(pointB1,closestB);

			var padding = 0;
			var haystackScale = 1.0;
			var haystackHeight = 31;
			var haystackWidthA = Math.round(lengthA);
			var haystackWidthB = Math.round(lengthB);
...
// console.log(haystackB);
				...
				var d = new DOImage(img);
				d.matrix().scale(sca);
				d.matrix().translate(1100, 150+haystackHeight);


				// d.matrix().translate(0 + lengthB-lengthA, 0);
				d.matrix().translate(0 + locationA-locationB, 0);
	...
	var rectifiedA = R3D.polarRectification(imageA,epipoleA);
		var infoA = rectifiedA;
		console.log(rectifiedA);
	rectifiedA = new ImageMat(rectifiedA["width"],rectifiedA["height"], rectifiedA["red"],rectifiedA["grn"],rectifiedA["blu"]);
	var rectifiedB = R3D.polarRectification(imageB,epipoleB);
		var infoB = rectifiedB;
		console.log(rectifiedB);
	rectifiedB = new ImageMat(rectifiedB["width"],rectifiedB["height"], rectifiedB["red"],rectifiedB["grn"],rectifiedB["blu"]);
	//var rectifiedB = R3D.polarRectification(imageB,epipoleB);
...
var offsetRectB = 10 + ( rotationB==0 ? -minRowB : -(rectifiedB.height()-minRowB) );
	...
	// find wheere imge rectification starts & ends
	/*
	iterate over perimeter & find lowest and highest angles
...
// GLOBALSTAGE.root().matrix().translate(0,100);
...
var pointB = new V2D(200,100);
var sizeA = 40;
var sizeB = 40;
var width = 100;
var height = 20;
var haystack = R3D.Dense.extractRectFromPoints(image,pointA,pointB, sizeA,sizeB, width,height);
...
R3D.BA.World.prototype._iteration = function(iterationIndex, isLastIteration){
	...
	for(var j=0; j<transforms.length; ++j){;
		...
		for(var i=0; i<maxMatchCount; ++i){ // get next best matches
			...
			var existingA = viewA.pointsInsideCellPoint(pointA.point(), true);
			var existingB = viewB.pointsInsideCellPoint(pointB.point(), true);

			// console.log(existingA);
			// console.log(existingB);

			if(existingA.length>0 || existingB.length>0){
				var nextScore = match.score();
				console.log("MATCH ALREADY EXISTS ... USE? : "+nextScore);
				var isBetterA = true;
				...
				var isBetter = isBetterA && isBetterA;
				if(isBetter){
					console.log("DROP PRIORS & USE THIS:");
					console.log(match);

					var imA = viewA.image();
					var pA = match.pointForView(viewA);
					var pB = match.pointForView(viewB);
					var a = pA.point();
					var b = pB.point();
/*
					var d = new DO();
					d.graphics().setLine(1, 0x99FF0000);
					d.graphics().beginPath();
					...
					d.graphics().setLine(1, 0x99FF0000);
					d.graphics().beginPath();
					d.graphics().drawCircle(b.x,b.y, 10 );
					d.graphics().strokeLine();
					d.graphics().endPath();
					d.matrix().translate(imA.width(), 0);
					GLOBALSTAGE.addChild(d);

*/
					if(existingA.length>0){
						existingA[0].point3D().disconnect(this);
					}
					if(existingB.length>0){
						existingB[0].point3D().disconnect(this);
					}
				...
				// throw "TODO: COMPARE / remove old socres if exits";
			...
			if(match){
				...
				this.appendNeighborMatchesAround(viewA, pointA.point(), viewB, pointB.point(), match);
				// this.checkP3DMadness();
			}
		}
	}
...
R3D.BA.World.prototype.appendNeighborMatchesAround = function(viewA, pointA, viewB, pointB, match){ // A look for best point in B & reverse
	this.appendNeighborMatchesSingle(viewA, pointA, viewB, pointB, match);
	this.appendNeighborMatchesSingle(viewB, pointB, viewA, pointA, match);
}
R3D.BA.World.prototype.appendNeighborMatchesSingle = function(viewA, pointA, viewB, pointB, match){ 
	var bestMatchesA = this.bestNextMatchesForPoint(viewA,pointA, viewB, match);
					...
					if(counterPoint){ // search around this point for good match
						...
						if(neighborsA){
							...
							if(viewB.isPointInside(pointB)){
								...
								// R3D.BA.setRankForMatch(match, true);
								if(match.rank()!==null){ // GET RANK TO MAKE SURE IT IS GOOD ENOUGH TO BE ADDED
									//var toPointArray
									/*
...
R3D.BA.World.prototype.addPointForMatch = function(bestMatchA, viewA, viewB, transform){
	...
	// rememeber params for next time:
	var rank = bestMatchA["rank"];
	// M
...
R3D.BA.World.SHOW = 0;
R3D.BA.World.prototype.bestNextMatchesForPoint = function(viewA, pointA, viewB, match){
	console.log("bestNextMatchesForPoint : "+pointA);
	var sourceScore = match.score();
...
sizeCompare = 11;
			...
			// var seedScales = Code.lineSpace(-.1,.1, .05);
			var seedAngles = Code.lineSpace(-20,20, 10);
			var seedScales = Code.lineSpace(-.1,.1, .1);
			// var seedAngles = Code.lineSpace(-25,25, 5);
			// var seedAngles = Code.lineSpace(-10,10, 5);
			// var seedScales = Code.lineSpace(-.1,.1, .1);
			// var seedAngles = null;
			// var seedScales = null;
// what if DNE
		...
		var subShow = show && i==1;
		var info = R3D.BA.optimumTransformForPoints(imageA,imageB, pA,pB, scaAB,angAB, sizeCompare, seedScales,seedAngles, searchWindow, subShow);
// console.log(info);
		if(!info){ // 
			continue;
			console.log("no optimum transform");
		}
		//var destinationScore = info["sad"];
		//var destinationScore = info["sad"];
		...
		var referenceA = scores["referenceA"];
			...
			var data = R3D.Dense.comparePathTransforms(imageA,pointA1,pointA2, imageB,pointB1,pointB2, sizeCompare,scaAB, subShow);
			var pathScore = 0;
			var referencePathA = 0;
			var referencePathB = 0;
			var pathScoreRelative = pathScore;

		var pA = info["from"];
		var pB = info["to"];
		if(!viewA.isPointInside(pA)){
			console.log("outside a: "+pA);
			continue;
		}
		if(!viewB.isPointInside(pB)){
			console.log("outside b: "+pB);
			continue;
		}
		// if(relative <= 1.5){
			// info["score"] = relative; // ...
			info["pathScoreAbsolute"] = pathScore;
			info["pathScoreRelative"] = pathScoreRelative;
			info["parentScoreAbsolute"] = sourceScore;
			info["neighborScoreAbsolute"] = destinationScore;
			info["referenceA"] = referenceA;
			info["referenceB"] = referenceB;
			info["referencePathA"] = referencePathA;
			info["referencePathB"] = referencePathB;

			var relativeCost = (pathScore/sourceScore);
			var relativeScore = (destinationScore/sourceScore);
			//var rank = relativeScore * relativeCost;
			// var rank = relativeCost;
			// var rank = pathScore * destinationScore;
				//var rank = ((destinationScore*destinationScore)/(referenceA*referenceB)) * (pathScore*pathScore/(referencePathA*referencePathB));
			// absolute cost * path-cost
			//var rank = destinationScore * pathScore;
			var rank = destinationScore * pathScore / sourceScore;

			info["rank"] = rank;
			bestMatches.push(info);

			info["score"] = destinationScore;
		// }
	}
...
// console.log(bestMatches);
	if(show){
console.log("sourceScore: "+sourceScore);
var feedback = [];
		for(var k=0; k<bestMatches.length; ++k){
			var m = bestMatches[k];
			var fr = m["from"];
			var to = m["to"];
			var an = m["angle"];
			var sc = m["scale"];
			var uq = m["uniqueness"];
				var referenceA = m["referenceA"];
				var referenceB = m["referenceB"];
				var referencePathA = info["referencePathA"];
				var referencePathB = info["referencePathB"];
				var tb = m["thisBest"];
				var nb = m["nextBest"];
				var scorePathA = m["pathScoreAbsolute"];
				var scorePathR = m["pathScoreRelative"];
				var sourceScore = m["parentScoreAbsolute"];
				var destinationScore = m["neighborScoreAbsolute"];
				var relScore = (destinationScore/sourceScore);
// feedback.push(scorePathA);
// feedback.push(destinationScore);
// feedback.push(destinationScore/referenceA);
// feedback.push(destinationScore/referenceB);
//feedback.push(relScore);
feedback.push(scorePathA*destinationScore);
			...
			var d = new DOText();
				...
				GLOBALSTAGE.addChild(d);
			var d = new DOText();
				d.text("path scores: "+sourceScore+" / "+scorePathA+" = "+scorePathR);
				d.align(DOText.ALIGN_LEFT);
				d.matrix().translate(1100 + sca*siz*2 + 10, 10 + k*siz*sca + siz*sca*0.25 + fnt*1);
				GLOBALSTAGE.addChild(d);

			//var scoreFinal = scorePathR * relScore;
			var scoreFinal = scorePathA * destinationScore;

			var d = new DOText();
				//d.text(" final score: "+scoreFinal);
				d.text(" final score: "+scoreFinal);
				d.align(DOText.ALIGN_LEFT);
				d.matrix().translate(1100 + sca*siz*2 + 10, 10 + k*siz*sca + siz*sca*0.25 + fnt*2);
				GLOBALSTAGE.addChild(d);

			var d = new DOText();
				d.text("    ..... uniqueness "+uq+" = "+" ... ");
				d.align(DOText.ALIGN_LEFT);
				d.matrix().translate(1100 + sca*siz*2 + 10, 10 + k*siz*sca + siz*sca*0.25 + fnt*3);
				GLOBALSTAGE.addChild(d);

			var d = new DOText();
				d.text("    ..... referenceA "+referenceA+" & "+referenceB+" = "+(destinationScore*destinationScore/(referenceA*referenceB)) );
				d.align(DOText.ALIGN_LEFT);
				d.matrix().translate(1100 + sca*siz*2 + 10, 10 + k*siz*sca + siz*sca*0.25 + fnt*4);
				GLOBALSTAGE.addChild(d);

			var d = new DOText();
				d.text("    ..... pathA "+referencePathA+" & "+referencePathB+" = "+(scorePathA*scorePathA/(referencePathA*referencePathB)) );
				d.align(DOText.ALIGN_LEFT);
			...
			var d = new DOText();
				d.text("  this: "+tb+" / "+nb+" = "+db);//+"  ... uniqueness: "+uq);
				d.align(DOText.ALIGN_LEFT);
				d.matrix().translate(1100 + sca*siz*2 + 10, 10 + k*siz*sca + siz*sca*0.5 + 60);
				GLOBALSTAGE.addChild(d);
			*/

		}
Code.printMatlabArray(feedback,"x");
		throw "throwed";
	}
...
//	var neighbors = space.objectsInsideRect(minRect,maxRect);
		...
		var neighbors = viewA.pointsInsideRect(minRect,maxRect, true);
			...
			if(d<=searchRadius){
				...
				if(val==0){
					...
					*/
					...
					if( V2D.distance(centerA, cellCenter)>1.0 ){ // WHY ?
