...
Stereopsis.World.prototype.refinePoint3DAbsoluteLocation = function(points){ // nonlinear P3D location optimization
...
Stereopsis.World.prototype.filterGlobalMatches = function(relax, iterationIndex, R,F,N,S){ // drops matches from transform (pairwise discarding)
...
/*
	...
	if(iterationIndex!==undefined && iterationIndex!==null){
		if(iterationIndex>5){
...
// console.log("MAKE UP EMPTY LOCATION:");
// emptyLocations = [new V2D(200,200)]; // couch
// emptyLocations = [new V2D(250,200)]; //BLACKNESS
// emptyLocations = [new V2D(250,60)]; // window corner
// emptyLocations = [new V2D(320,50)]; // shelf thing
// emptyLocations = [new V2D(320,250)]; // table gray area
// emptyLocations = [new V2D(420,270)]; // carpet
// emptyLocations = [new V2D(480,250)]; // chair
// emptyLocations = [new V2D(100,310)]; // couch corner
// emptyLocations = [new V2D(100,100)];
var edgeLocations = [];
			var matchesAddList = [];
...
Stereopsis.World.prototype.embedPoint3D = function(point3D, validCheck){ // insert the P3D into the various views -> checks for intersection first (and calls resolve intersection)
	...
	if(checkIntersection){
		for(var i=0; i<views.length; ++i){
			var view = views[i];
			var cellSize = view.cellSize();
			var minDistance = Math.min(cellSize*Stereopsis.World.MIN_DISTANCE_EQUALITY,Stereopsis.World.MIN_DISTANCE_EQUALITY_MIN);
...
Stereopsis.World.prototype._resolveIntersectionPatch = function(point3DA,point3DB){
	...
	if( !(loadedViewsA.length>1 && loadedViewsB.length>1) ){
		console.log("not enough loaded views to compare");
	...
	var limitA = scoresA["mean"] + scoresA["sigma"] * 1.0;
	var limitB = scoresB["mean"] + scoresB["sigma"] * 1.0;
	var checkC = scoresC["mean"];
	var minimumCut = 0.10; // TODO: REALLY GOOD PAIRS CAN BE OVER-PROTECTIVE
...
Stereopsis.optimumScoresAtLocation = function(imageA,pointA, imageB,pointB, needleSize,haystackRelativeSize, matrix){ // search needle/haystack at points
	var compareSize = Stereopsis.COMPARE_SIZE;
	var cellScale = (needleSize/compareSize); // EG:
