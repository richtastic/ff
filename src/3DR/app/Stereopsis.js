...
Stereopsis.View.prototype.addViewProbePoint = function(myPoint, oppositeView){//, oppositePoint){
...
Stereopsis.View.prototype.bestCorner = function(cellX,cellY){
	...
	// console.log(" . . . . . "+this.cellCenter(cellX,cellY)+" = "+point);	
	return point;
}
...
orderedPoints.sort(function(a,b){
	...
	// console.log("T - "+rMean+" @ "+rHalf+" +/- "+rSigma);
...
Stereopsis.P3D.prototype.calculateAbsoluteLocation = function(world, transformIn){
	...
	if(components.length==0){
		console.log("calculateAbsoluteLocation components = 0");
...
Stereopsis.World.prototype._iterationSolveAbs = function(iterationIndex, maxIterations){
	...
	for(var i=0; i<refineCount; ++i){
		// relative R from absolutes
		this.relativeTransformsFromAbsoluteTransforms();
		// relative F derived from relative R
		this.relativeFfromR();
		// 
		this.estimate3DErrors(true);
	...
	// PROBING
	this.probe3D(); // before 2D: want 3D location to project
	this.probe2D();
	
	// NEW POINTS BRING NEW ERRORS

	// TRANSFORM R/F/M FILTER
	this.filterGlobalMatches(false);
	...
	// this.filterLocal2D(); /// makes things worse
	this.filterLocal3D();
...
Stereopsis.World.prototype.relativeTransformsFromAbsoluteTransforms = function(){
	...
	for(var i=0; i<transforms.length; ++i){
		...
		var viewA = transform.viewA();
		var viewB = transform.viewB();
		var relative = this.relativeTransformFromViews(viewA,viewB);
...
Stereopsis.World.prototype.relativeTransformFromViews = function(viewA,viewB){
	var absB = viewB.absoluteTransform();
	var invA = viewA.absoluteTransformInverse();
	var relativeAtoB = Matrix.mult(invA,absB);
	return relativeAtoB;
}
Stereopsis.World.prototype.refineCameraAbsoluteOrientation = function(){
	var minimumMatches = 16; // maybe also PERCENTAGE of average nonzer / kNN | eg: ~10%
	var views = this.toViewArray();
	// var transforms = this.toTransformArray();
	// console.log(transforms);
	var viewNewLocations = [];
	for(var i=0; i<views.length; ++i){
		...
		for(var j=0; j<views.length; ++j){
			...
			if(matchCount>minimumMatches){
				var estimate = this.bundleAdjustViewPair(viewB,viewA);
				var absoluteA = estimate["P"];
		...
		for(var j=0; j<estimates.length; ++j){
			matrixes.push(estimates[j]["P"]);
			errors.push(estimates[j]["error"]);
		}
		var estimate = this.averageMatrixEstimates(matrixes, errors);
		viewNewLocations.push(estimate);
	}
	// set new locations
	for(var i=0; i<views.length; ++i){
		var viewA = views[i];
		var estimate = viewNewLocations[i];
		// console.log("FROM: \n"+viewA.absoluteTransform());
		viewA.absoluteTransform(estimate);
		// console.log("  TO: \n"+viewA.absoluteTransform());
	}
}
Stereopsis.World.prototype.averageMatrixEstimates = function(matrixes, errors){ // 
...
Stereopsis.World.prototype.bundleAdjustViewPair = function(viewA,viewB){ // where viewB should be using viewA + relative tansform points
	...
	for(var k=0; k<matches.length; ++k){
		...
		// var point3D = match.estimated3D(); // this is only for the relative pair -> needs to be offset by absolute A
		var point3D = match.point3D();
	...
	// console.log("POINTS: "+points3D.length+" / "+matches.length);
	...
	var result = R3D.BundleAdjustCameraExtrinsic(Ks, Is, Ps, points2D, points3D, maxIterations);
	var error = result["error"];
	var count = result["count"];
	var extrinsics = result["extrinsics"];
	var averageError = error/count;
	var absA = extrinsics[0];
	var absB = extrinsics[1];
	return {"P":extrinsics[1], "error":averageError};
}
			//
Stereopsis.World.prototype.refinePoint3DAbsoluteLocation = function(){
	...
	for(var i=0; i<transforms.length; ++i){
		...
		var KaInv = viewA.Kinv();
		var KbInv = viewB.Kinv();
		var relAB = transform.R(viewA,viewB);
		var matches = transform.matches();
		for(var j=0; j<matches.length; ++j){
			var match = matches[j];
			...
			// console.log(pA+"");
			// console.log(pB+"");
			var estimated3D = R3D.triangulatePointDLT(pA,pB, absA,absB, KaInv, KbInv);
			// console.log("WAS: "+match.estimated3D());
			match.estimated3D(estimated3D);
			// console.log("IS: "+match.estimated3D());
			// throw "...";
		}
		// throw "...";
	}
...
Stereopsis.World.prototype.averagePoints3DFromMatches = function(onlyNull){
	// average locations into final location
	var points3D = this.toPointArray();
	for(var i=0; i<points3D.length; ++i){
		var point3D = points3D[i];
		...
		for(var j=0; j<matches.length; ++j){
			...
			// console.log(error);
			locations.push(match.estimated3D());
			errors.push(error);
		}
		...
		this.updatePoint3DLocation(point3D,location);
	}
...
//console.log(pointSizeA); // similar sizes -- [.10, .12,.14, .17]
			...
			for(var j=0; j<viewsA.length;++j){
				...
				//console.log(searchPoints.length+" - "+pointDirViewA.length()+" / "+pointCenterA+" of "+searchRadius);
				// console.log(searchPoints.length);
	...
	// limits
	var limF = Math.floor(avgF + sigF*3.0);
	var limB = Math.floor(avgB + sigB*3.0);
...
Stereopsis.World.prototype.filterLocal3D = function(){ 
	var points3D = this.toPointArray();
	var views = this.toViewArray();
	var neighborCount = 9; // 5-9
	var invNeighborCount = 1.0/neighborCount;
// var distances = [];
	...
	for(var i=0; i<views.length; ++i){
		...
		for(var j=0; j<points2D.length; ++j){
			...
			if(neighbors.length==neighborCount){
				...
				var sigD = Code.stdDev(d, minD);
				var point3D = point2D.point3D();
				var p3D = point3D.point();
				var maxPointDistance = avgD + sigD*2.0; // 3 gets nothing
				var pointDistance = V3D.distance(p3D,centroid);
// distances.push(avgD/minD);
...
// Code.addUnique(dropList,point2D);
	...
	// 	return a < b ? -1 : 1;
	// });
	// Code.printMatlabArray(distances,"x");
	// drop worst
...
Stereopsis.World.prototype.possiblyVisibleViews = function(pointCenter, pointNormal, visibleViews){
	...
	for(var i=0; i<views.length; ++i){
		...
		if(distanceAverage){
			...
			// console.log(" DIST 1: "+distance+" / "+distanceAverage);
...
Stereopsis.World.prototype.probe3D = function(){ // requires patches to orientate projections
	...
	// all
// TODO: POSSIBLY ALSO IGNORE NON-UNIQUE POINTS | BAD-CORNERENESS
		...
		if(point3D.hasPatch()){
			...
			if(possibleViews.length>0){ // exist more views to possibly check
				...
				for(var j=0; j<fullList.length; ++j){
					...
					var image = view.image();
					var cellSize = view.cellSize();
					var minArea = cellSize * 0.5;
					var maxArea = cellSize * 2.0;
					// var K = view.K();
					// var P = view.absoluteTransform();
					// var distortions = null;
					// var projected2D = R3D.projectPoint3DToCamera2DForward(p3D, P, K, distortions);
					...
					// projected patch can't be too big
					var area = right2D.length()*up2D.length();
					area = Math.sqrt(area);
...
*/
				...
				// console.log("CURR: @ "+minCurr+" +/- "+sigCurr);
				for(var j=0; j<possibleCount; ++j){
					...
					if(avgNext<limNext){
						...
						// console.log("create new match @ point: "+minIndex);
						// console.log(viewA,pointA,viewB,pointB, affine);
						...
						//var match = this.addMatchFromInfo(viewA,pointA,viewB,pointB, affine, null, true);
...
GLOBALSTAGE.addChild(d);
	...
	console.log(" => ADDING NEW MATCHES: "+newMatches.length);
	// throw "OUT";
...
// tracking.push([match,bestMatch,viewA,viewB]);
					}
				}
			}
		}
...
// console.log("NEW MATCH: "+viewA.id()+"-"+viewB.id()+" = "+match.errorNCC()+" "+match.errorSAD()+"");
	...
	// console.log("   2 "+averagePoint3D+" @ "+pA3D+" & "+pB3D);
	// this.validatePoints3DX();
	// console.log("AFTER");
	// console.log(point3DC.toMatchArray().length);
	// cleanup - new
	for(var i=0; i<pointsList.length; ++i){
...
Stereopsis.World.prototype.removeMatchFromPoint3D = function(match){
	...
	// console.log(removePoint2D.view())
...
// viewA.insertPoint2D(point2D); // ???
		for(var j=i+1; j<points2D.length; ++j){
			...
			viewA.addViewProbePoint(point2DA.point2D(), viewB);//, point2DB.point2D());
			viewB.addViewProbePoint(point2DB.point2D(), viewA);//, point2DA.point2D());
		}
	}
	// point3D
...
Stereopsis.World.prototype.disconnectPoint3D = function(point3D){
	...
	for(var i=0; i<points2D.length; ++i){
		...
		// console.log(point2D.point2D()+" = "+removed);
	}
	//
	// viewA.removeViewProbePoint(point2DA.point2D(), viewB);//, point2DB.point2D());
	// viewB.removeViewProbePoint(point2DB.point2D(), viewA);//, point2DA.point2D());
	// point3D
...
Stereopsis.World.prototype.bundleAdjustFull = function(){
return;
// NOT CHECKED YET
