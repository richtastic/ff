...
Stereopsis.World.prototype.refineSelectCameraMultiViewTriangulation = function(selectView, maxIterations){ 
	...
	for(var i=0; i<points2D.length; ++i){
		...
		if(pointsP3DP2D.length>2){ // more than 1 match
...
Stereopsis.World.prototype.solveDenseGroup = function(inputPairs){ // multiple (3+)
	...
	for(var i=0; i<inputPairs.length; ++i){
		...
		var transformA = viewA.absoluteTransform();
		var transformB = viewB.absoluteTransform();
...
Stereopsis.World.prototype.probe2DCells = function(sigmaExistingRFN, initPatches){
	var world = this;
