...
Stereopsis.Transform3D.prototype.toBestPointArray = function(percentR, percentF, percentN, percentS, percentC){ // R & F may be noncalculated
	var minimumMatchCount = 16;
	percentR = Code.valueOrDefault(percentR, 0.5);
	percentF = Code.valueOrDefault(percentF, 0.5);
	percentN = Code.valueOrDefault(percentN, 0.5);
	percentS = Code.valueOrDefault(percentS, 0.5);
	// C
	var matches = this.matches();
	var viewA = this.viewA();
	var viewB = this.viewB();
...
// Code.printMatlabArray(listS,"S");
	...
	console.log(limitR+" : "+listR[0]+" - "+listR[listR.length-1]);
	console.log(limitF+" : "+listF[0]+" - "+listF[listF.length-1]);
	console.log(limitN+" : "+listN[0]+" - "+listN[listN.length-1]);
	console.log(limitS+" : "+listS[0]+" - "+listS[listS.length-1]);
...
Stereopsis.World.prototype.printPoint3DTrackCount = function(){
	...
	var totalArrayCount = R3D.maxiumMatchesFromViewCount(totalViewCount) + 2;
	var countList = Code.newArrayZeros(totalArrayCount);
	var points3D = this.toPointArray();
// console.log(totalArrayCount);
...
Stereopsis.World.prototype.calculateAbsoluteLocationsPoint3D = function(){ // average match locations into final averaged location
	var world = this;
	var points3D = this.toPointArray();
	for(var i=0; i<points3D.length; ++i){
		var point3D = points3D[i];
...
Stereopsis.World.prototype.solveOptimizeSingleView = function(viewSolve){ // move view to a more optimal position
	...
	var iterationsMain = 500;
	var iterationsNeighbors = 100;
	// var views = world.toViewArray();
	// viewSolve = views[0];
...
// world.refineSelectCameraAbsoluteOrientationTriangulate(viewSolve);
// world.copyRelativeTransformsFromAbsolute();
// world.estimate3DErrors(true, false);

// world.refineSelectCameraMultiViewTriangulation(viewSolve);
// world.copyRelativeTransformsFromAbsolute();

		// 3+ track groups:
		var result = world.refineSelectCameraMultiViewTriangulation(viewSolve, iterationsMain);
		// if(result==false){ // can't run
		// 	break;
		// }
		world.copyRelativeTransformsFromAbsolute();
...
Stereopsis.World.prototype.refineSelectCameraAbsoluteOrientationTriangulate = function(selectView, maxIterations){ 
	...
	for(var i=0; i<views.length; ++i){
	 ...
	 // todo: some king of sigma to limit to only most influential views (5-10)
...
Stereopsis.World.prototype.dropWorstParametersF = function(){ // ..
	...
	for(var i=0; i<views.length; ++i){
		...
		var limitC = Code.percentile(values,1.0-percentKeepC);
		for(var j=0; j<points2D.length; ++j){
			var point2D = points2D[j];
...
Stereopsis.World.prototype.iteration = function(iterationIndex, maxIterations, data){
	...
	if(isFirst){ // SET INITIAL VIEW CAMERA EXTRINSICS
		...
		world.calculateAbsoluteLocationsPoint3D();
...
Stereopsis.World.prototype.subdivideViewGrids = function(scaleRatio, minCellSize){
	...
	// update patches with new cell size
	TODO
...
// throw "..."
	...
	for(var i=0; i<inputPairs.length; ++i){
		...
		var viewA = pair[0];
		var viewB = pair[1];
		console.log("PAIR CREATING +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ "+i+" / "+inputPairs.length+" ("+viewA.data()+"-"+viewB.data()+") ...");
		var absoluteA = viewA.absoluteTransform();
		var absoluteB = viewB.absoluteTransform();
		var extrinsicAB = Matrix.relativeWorld(absoluteA,absoluteB);
		var imageA = viewA.image();
		var imageB = viewB.image();
		...
		// var sizes = [viewA.cellSize(),viewB.cellSize()];
		var errorPixels = imageA.size().length()*errorSize;
...
console.log("refine b - refineSelectCameraMultiViewTriangulation")
		...
		world.dropNegativePoints3D();
		world.dropFurthest();
		// world.filterLocal3Dto2DSize(); // N/A
		world.filterNeighborConsistency();
		// world.filterLocal3D(); // ...
...
Stereopsis.World.prototype.filterNeighborConsistency = function(){ // drop 3D outlier noise
	...
	var worstNeighborPercent = 0.25; // progression: 0.10 => 0.50 : (forgiving) => (strict)
	var world = this;
	var transforms = world.toTransformArray();
...
Stereopsis.ransacTransformF = function(transform, maximumSamples, skipP){ // F & P
	...
	var info = transform.toBestPointArray(0.75,0.75,0.5,0.5, 0.5); // care more about N & S & C -- 14%
	var viewA = transform.viewA();
	var viewB = transform.viewB();
...
// sigmaMaximumSelect = 999.0;
	...
	for(var i=0; i<transforms.length; ++i){
		...
		var maxCheckErrorF = errorFmean + sigmaMaximumSelect*errorFsigma; // empties worth trying
		var maxCheckErrorN = errorNmean + sigmaMaximumSelect*errorNsigma;
		var maxCheckErrorS = errorSmean + sigmaMaximumSelect*errorSsigma;
		var maxNewErrorF = errorFmean + sigmaMaximumNew*errorFsigma; // new matches worth keeping
		var maxNewErrorN = errorNmean + sigmaMaximumNew*errorNsigma;
		var maxNewErrorS = errorSmean + sigmaMaximumNew*errorSsigma;
		console.log("allowed max errors: F: "+maxCheckErrorF+" | N: "+maxCheckErrorN+" | S: "+maxCheckErrorS);
		...
		for(var v=0; v<viewsA.length; ++v){
			...
			var featureSize = (viewA.compareSize()+viewB.compareSize())*0.5;
				// ...
			// console.log("compareSize: "+featureSize);
			// var emptyCells = viewA.emptyCells();
					...
					if(newMatch){
						...
						if(keep){
							...
							if(uniquenessScore<maximumUniquenessScore){
								...
								var pointA = point2DA.point2D();
								var pointB = point2DB.point2D();
								// console.log(pointA,pointB, imageA,imageB, featureSize,compareSize, angle,scale);
								...
								*/
								...
								var compareSize = Math.min(Math.max(featureSize,3),7); // 5 - 9
								var maxIterations = 15;
	...
	/*
		for each transform:
			for each view A->B & B->A
				find empty cells next to a filled cell
				for each empty cell:
					get list of NN - cell based (points with views A&B)
					for each NN:
						- must pass sigma min F & N & S
						- keep closestpoint
					if a best neighbor exists:
						- get rotation&scale from affine
						- find best predicted location in opposite view
						- optimize local affine
						- create new match
						- add match to new-match-list
				add new matches for view
	*/
}
Stereopsis.World.prototype.subDivideUpdateMatchLocation = function(){ // 2D update, matches+affine change in position
...
Stereopsis.World.prototype.filterLocal2DAffineError = function(){ // locally drop points with high error:
	...
	for(var t=0; t<transforms.length; ++t){
		...
		for(var v=0; v<viewsA.length; ++v){
			...
			var viewA = viewsA[v];
			var viewB = viewsB[v];
			var maxSearchRadiusA = viewA.cellSize() * 2; //
			var spaceA = viewA.pointSpace();
...
Stereopsis.World.prototype.filterGlobal3DR = function(sigmaMaximumDrop){
	// 
	console.log("filterGlobal2DR - global R / F / N");
	//
}
Stereopsis.World.prototype.filterGlobal2DF = function(sigmaMaximumDrop){ // globally drop points with high error:
...
// A
// 11 & 22 extract (605)
// 11x11 x 11x11 checks (14641)

// B
// 5 & 10 & 11 & 13 extract (415)
// 5x5 x 5x5 checks (625)
// 11x11 x 3x3 checks (1089)
// 
// 7x7 x 7x7 (2401)
	
	// B needle / haystack increase accuracy
	// POOR
	// var needleSizes = [5,11];
	// var haystackSizes = [5*2,11+2];
	// var needleSizes = [7,11];
	// var haystackSizes = [7*2,11+2];
	// var result = R3D.optimumSADLocationSearchFlatRGBIteritive(existingA,predictedB, viewA.imageScales(),viewB.imageScales(), featureSize, needleSizes,haystackSizes, affineAB);
