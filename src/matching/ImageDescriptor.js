...
//u = Matrix.inverse(u);
		...
		U = Matrix.mult(u,U); // concatenation u * U
		//U = Matrix.mult(U,u);
...
var vectorMin = new V2D(0,0);
var vectorMax = new V2D(0,0);
if(eig.values[0]>eig.values[1]){
	lambdaMax = eig.values[0];
	lambdaMin = eig.values[1];
...
amt = (separation.scaleX+separation.scaleY)*0.5;
amt = 1/amt;
...
// //correctScale = originalScale*correctScale;
// correctScale = (originalScale/x.z)*correctScale;
...
// var mXSta = Math.floor(winWid*0.5)-3;
// var mXEnd = Math.floor(winWid*0.5)+3;
// var mYSta = Math.floor(winHei*0.5)-3;
// var mYEnd = Math.floor(winHei*0.5)+3;
// var mXLen = mXEnd - mXSta + 1;
// var mYLen = mYEnd - mYSta + 1;
// var m1 = Code.subArray2D(measure,winWid,winHei, mXSta,mXEnd, mYSta,mYEnd);
// console.log(Code.toStringArray2D(m1,mXLen,mYLen));
	// get peaks
	var peaks = ImageMat.findExtrema2DFloat(measure, winWid,winHei, 1.0,1.0, 1E-10); // this doesn't work as expected
