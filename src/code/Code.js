...
Code.PolyPoint.prototype.next = function(n){
	if(n!==undefined){
		this._next = n;
	}
	return this._next;
}
Code.PolyPoint.prototype.prev = function(p){
	if(p!==undefined){
		this._prev = p;
	}
	return this._prev;
...
Code.PolyEdge.prototype.right = function(r){
	if(r!==undefined){
		this._right = r;
	}
	return this._right;
}
Code.PolyEdge.prototype.next = function(n){
	if(this._right){
		return this._right.next();
	}
	return null;
}
Code.PolyEdge.prototype.prev = function(p){
	if(this._left){
		return this._left.prev();
	}
	return null;
}
// --------------------------------------------------------------------------------------- polygon edge chain
...
Code.edgeListFromPolygon = function(poly){
	...
	var point, points = [];
	for(i=0;i<lenA;++i){
		point = new Code.PolyPoint(poly[i]);
		points.push(point);
	}
	for(i=0;i<lenA;++i){
		a = points[i];
...
Code.PolySweepLine.prototype.next = function(event){
	event = this._edges.next(event);
	return event;
}
Code.PolySweepLine.prototype.prev = function(event){
	event = this._edges.prev(event);
	return event;
}
Code.PolySweepLine.prototype.erase = function(event){
...
// --------------------------------------------------------------------------------------- 
Code.PolySweepPointEvent = function(edg,pnt,ply,etp){
...
Code.PolySweepPointEvent.EdgeTypeRegular = 1;
Code.PolySweepPointEvent.EdgeTypeNonContributing = 2;
Code.PolySweepPointEvent.EdgeTypeSameTransition = 3;
Code.PolySweepPointEvent.EdgeTypeDifferentTransition = 4;
Code.PolySweepPointEvent.ResultTypeUnknown = 0;
Code.PolySweepPointEvent.ResultTypeUnion = 1;
Code.PolySweepPointEvent.ResultTypeIntersection = 2;
Code.PolySweepPointEvent.ResultTypeDifference = 3;
Code.PolySweepPointEvent.ResultTypeXOR = 3;
...
Code.PolySweepPointEvent.prototype.left = function(l){
	if(l!==undefined){
		this._left = l;
	}
	return this._left;
...
Code.PolySweepPointEvent.prototype.isLeftPoint = function(){
	...
	if(this._opposite){
		...
		var o = this.opposite().point().point();
		if(p.x<o.x){
			return true;
		}
		if(p.x>o.x){
		...
		return this._edge.left() == this.point(); // same point
	}
	return false;
...
Code.PolySweepPointEvent.prototype.inOut = function(i){
	if(i!==undefined){
		this._inOut = i;
	}
	return this._inOut;
}
Code.PolySweepPointEvent.prototype.polyType = function(p){
	if(p!==undefined){
		this.polyType = p;
	}
	return this.polyType;
}
Code.PolySweepPointEvent.prototype.edgeType = function(e){
	if(e!==undefined){
		this._edgeType = e;
	}
	return this._edgeType;
}
Code.PolySweepPointEvent.prototype.isEdgeBelowPoint = function(point){
...
Code.PolySweepPointEvent.prototype.isEdgeAbovePoint = function(point){ // rename to aboveorequalto
	var area = V2D.areaTri(this.point().point(),this.opposite().point().point(), point);
	if(this.isLeftPoint()){
		return area >= 0;
	}
...
Code._dividePolySegment = function(eventA,point,sweep,queue){
	var p = new Code.PolyPoint(new V2D(point.x,point.y));
	var edgeA = eventA.edge();
	var pLeft = edgeA.left();
	var pRight = edgeA.right();
	var edgeB;
	if( V2D.equal(eventA.point().point(),pLeft.point()) ){
		edgeB = new Code.PolyEdge(p,pRight);
		edgeA.right(p);
	}else{
		edgeB = new Code.PolyEdge(pLeft,p);
		edgeA.left(p);
	}
	if(pLeft.next()==pRight){
		...
		pLeft.next(p);
		pRight.prev(p);
	}else{ // pLeft.next()==pLeft
		p.prev(pRight);
		p.next(pLeft);
		pRight.next(p);
		pLeft.prev(p);
	}
	var eventBO = eventA.opposite();
	eventBO.edge(edgeB);
	var eventAO = new Code.PolySweepPointEvent(edgeA, p, eventA.polygon(), eventA.edgeType());
	var eventB  = new Code.PolySweepPointEvent(edgeB, p, eventBO.polygon(), eventBO.edgeType());
	eventA.opposite(eventAO);
	eventAO.opposite(eventA);
	eventBO.opposite(eventB);
	eventB.opposite(eventBO);

	queue.push(eventAO);
	queue.push(eventB);
	// DO CHECK:
	if( !(eventA.opposite()==eventAO) ){
		console.log("DIVIDE ERROR: A");
	}
	if( !(eventA==eventAO.opposite()) ){
		console.log("DIVIDE ERROR: AO");
	}
	if( !(eventB.opposite()==eventBO) ){
		console.log("DIVIDE ERROR: B");
	}
	if( !(eventB==eventBO.opposite()) ){
		console.log("DIVIDE ERROR: BO");
	}
	if( !(eventA.edge()==edgeA) ){
		console.log("DIVIDE ERROR: AE");
	}
	if( !(eventAO.edge()==edgeA) ){
		console.log("DIVIDE ERROR: AO");
	}
	if( !(eventB.edge()==edgeB) ){
		console.log("DIVIDE ERROR: BE");
	}
	if( !(eventBO.edge()==edgeB) ){
		console.log("DIVIDE ERROR: BO");
	}
	if( !(V2D.equal(eventA.point().point(),edgeA.left().point()) || V2D.equal(eventA.point().point(),edgeA.right().point()) ) ){
		console.log("DIVIDE ERROR: AP");
		console.log(" "+eventA.point().point()+" | "+edgeA.left().point()+" | "+edgeA.right().point());
	}
	if( !(V2D.equal(eventAO.point().point(),edgeA.left().point()) || V2D.equal(eventAO.point().point(),edgeA.right().point()) ) ){
		console.log("DIVIDE ERROR: AOP");
		console.log(" "+eventAO.point().point()+" | "+edgeA.left().point()+" | "+edgeA.right().point());
	}
	if( !(V2D.equal(eventB.point().point(),edgeB.left().point()) || V2D.equal(eventB.point().point(),edgeB.right().point()) ) ){
		console.log("DIVIDE ERROR: BP");
	}
	if( !(V2D.equal(eventBO.point().point(),edgeB.left().point()) || V2D.equal(eventBO.point().point(),edgeB.right().point()) ) ){
		console.log("DIVIDE ERROR: BOP");
		console.log(" "+eventBO.point().point()+" | "+edgeB.left().point()+" | "+edgeB.right().point());
	}
}

Code._possibleEdgeIntersection = function(eventA,eventB, sweep, queue){ // intersection between two edges   |||||||||||||||| extend polygon to be set of arrays: groups of polygons
	...
	if(!isSamePoly){
		...
		if(point){
			var numIntersections = 1;
			if( Code.isArray(point) ){ // collinear intersection
				console.log("HANDLE COLLINEAR INTERSECTION");
				numIntersections = 2;
			}
			// if( V2D.equal(a,c) || V2D.equal(b,d) ){ // intersect at same start point // HANDLED BY ISINTERSECTOINPOINT
			// 	console.log("WOULD NOT INTERSECT 1");
			// }
			var isIntersectionEndPoint = V2D.equal(a,c) || V2D.equal(a,d) || V2D.equal(b,c) || V2D.equal(b,d);
			if (!isIntersectionEndPoint){
				console.log("FOUND INTERSECT: "+point);
				isIntersectionEndPoint = V2D.equal(point,a) || V2D.equal(point,b) || V2D.equal(point,c) || V2D.equal(point,d);
				// subdivide lines
				var edgeA = eventA.edge();
				...
				if(false){ // collinear dual-line subdivision
					...
					if( !V2D.equal(point,a) && !V2D.equal(point,b) ){
						Code._dividePolySegment(eventA, point, sweep, queue);
					}
					if( !V2D.equal(point,c) && !V2D.equal(point,d) ){
						Code._dividePolySegment(eventB, point, sweep, queue);
					}
				}
				return true;
			}/*else{
...
Code._polygonSweepEventSortBoolean = function(a,b){ // true == after(-1), false == before(1)
	var app = a.point().point();
	var bpp = b.point().point();
	if( app.x > bpp.x ){
		return true;
	}
	if( app.x < bpp.x ){
	...
	var bop = b.opposite().point().point();
	return a.isEdgeAbovePoint(bop);
}
Code._polygonSweepEventSort = function(a,b,s){ // a & b are ONLY left events
	if(a==b){ return 0; }
	var app = a.point().point();
	var bpp = b.point().point();
	var aop = a.opposite().point().point();
	var bop = b.opposite().point().point();
	
	var appAbovebpp = a.isEdgeAbovePoint(bpp);
	var appAbovebop = a.isEdgeAbovePoint(bop);
	var bppAboveapp = b.isEdgeAbovePoint(app);
	var bppAboveaop = b.isEdgeAbovePoint(aop);

	var appBelowbpp = a.isEdgeBelowPoint(bpp);
	var appBelowbop = a.isEdgeBelowPoint(bop);
	var bppBelowapp = b.isEdgeBelowPoint(app);
	var bppBelowaop = b.isEdgeBelowPoint(aop);

	var signedAreaA = V2D.areaTri(app,aop,bpp);
	var signedAreaB = V2D.areaTri(app,aop,bop);
	var notCollinear = signedAreaA!=0.0 || signedAreaB!=0.0;
	if(notCollinear){
		if(V2D.equal(app,bpp)){ // sort on right endpoint
			return a.isEdgeBelowPoint(bop) ? -1 : 1;
		}
		if( Code._polygonSweepEventSortBoolean(a,b) ){
			return b.isEdgeAbovePoint(app) ? -1 : 1;
		}
		return a.isEdgeBelowPoint(bpp) ? -1 : 1;
	} // else collinear
	console.log("COLLINEAR POINTS FOUND:");
	console.log("A: "+a);
	console.log("B: "+b);
	if(V2D.equal(app,bpp)){ 
		return (a.id()<b.id()) ? -1 : 1;
	}
	return Code._polygonSweepEventSortBoolean(a,b) ? -1 : 1;
}
...
// --------------------------------------------------------------------------------------- 
Code._polyInitProcessEdge = function(edge, type, sweep, queue){
	if( V2D.equal(edge.left().point(),edge.right().point()) ){
		return; // degenerate
	}
	var eventA = new Code.PolySweepPointEvent(edge, edge.left(),  type);
	var eventB = new Code.PolySweepPointEvent(edge, edge.right(), type);
...
Code.polygonUnion2D = function(polyA,polyB, _iteration, operation){
	operation = Code.PolySweepPointEvent.ResultTypeUnion;
	if(polyA.length==0 && polyB.length==0){ // nothing to do, empty result
		return [];
	}
	...
	var queue = new PriorityQueue(); // list of left and right sweep events
	var sweep = new Code.PolySweepLine(); // list of left sweep events
	var intersect = []; // ?
	var union = []; // ?
	...
	for(i=0; i<len; ++i){
		edge = edgesA[i];
		Code._polyInitProcessEdge(edge,Code.PolySweepPointEvent.PolygonTypeSubject, sweep,queue);
	}
	// add polygon B points
	len = edgesB.length;
	for(i=0; i<len; ++i){
		edge = edgesB[i];
		Code._polyInitProcessEdge(edge,Code.PolySweepPointEvent.PolygonTypeClipped, sweep,queue);
	}
	// iterate thru all (dynamic) left and right events in queue
var iter = 0;
	while( !queue.isEmpty() ){
		console.log("..................................................."+iter+"/"+_iteration);
		event = queue.minimum();
		queue.removeObject(event); // queue.popMinimum();
		if(event.isLeftPoint()){
			...
			foundIntersection |= Code._possibleEdgeIntersection(event, next, sweep, queue);
			foundIntersection |= Code._possibleEdgeIntersection(prev, event, sweep, queue);
		}else{ // right endpoint
			console.log("...right");
			opposite = event.opposite();
			next = sweep.next(opposite);
			prev = sweep.prev(opposite);
			if( opposite.isInsideOtherPolygon() ){
				intersect.push(event.edge());
			}
			if( !opposite.isInsideOtherPolygon() ){
				union.push(event.edge());
			}
			sweep.erase(opposite);
			Code._possibleEdgeIntersection(prev, next, sweep, queue);
		}
		iter++;
		if(iter>_iteration && true){
			break;
		}
	}
	if(!foundIntersection){ // if polygons have no intersection, return: empty, union, A-B
		if(operation==Code.PolySweepPointEvent.ResultTypeUnion){
			// union
		}else if(operation==Code.PolySweepPointEvent.ResultTypeIntersection){
			// intersect
		}else if(operation==Code.PolySweepPointEvent.ResultTypeDifference){
			// difference
		}else{
			return [];
		}
...
/*
Code.edgeListFromPolygonOLD = function(poly){
...
Code.polygonUnion2D_OLD = function(polyA,polyB){
...
edgeOld.intersect = intersect;
			...
			edgeNew = {"start":point, "end":edgeOld.end, "next":next, "prev":edgeOld, "intersections":intersections, "processed":false};
			edgeOld.end = point;
